...
function getFullNames(customers){
   var strArray = []

   for (var i = 0; i < customers.length; i++){
      var custName = ''

      custName = customers[i].first + ' ' + customers[i].last

      strArray.push(custName)
   }
   return strArray
}
...
function createListOfObjects(realNinjas) {
    var objArray = []
    ...
    for (var i = 0; i < realNinjas.length; i++) {
        ...
        ninjaArr = realNinjas[i].split(' ')
        obj.firstName = ninjaArr[0]
        obj.lastName = ninjaArr[1]
    ...
    var dogObj = {
        ...
        speak: function(str){
            ...
            for (var i = 0; i < strArr.length; i++) {
                var indivLetters = strArr[i].split('')


                indivLetters[0] = 'r'
                var newStr = indivLetters.join('')
...
function pluck (arrOfObj, propName) {
    var finalArr = []

    for (var i = 0; i < arrOfObj.length; i++) {
        var itemObj = arrOfObj[i]

        if (propName === 'name') {
        	finalArr.push(itemObj.name)
        } else if (propName === 'age') {
            finalArr.push(itemObj.age)
        } else if (propName === 'hairStyle'){
            finalArr.push(itemObj.hairStyle)
        }
	}
    return finalArr
}
...
function getCounts(sampleText) {
    var wordCountObj = {}
    var arrOfWords = sampleText.toLowerCase().split(' ')

    for (var i = 0; i < arrOfWords.length; i++) {
        var theWord = arrOfWords[i].match(/[^_\W]+/g).join('')
